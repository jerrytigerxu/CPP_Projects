...
Static Linking Note
The user's original plan mentioned creating a "distributable, standalone executable using static linking."

nlohmann/json is a header-only library, so it's always "statically" included.

libcurl is more complex. True static linking of libcurl involves linking not only libcurl.a but also all of its dependencies (e.g., OpenSSL/LibreSSL for HTTPS, zlib for compression, nghttp2 for HTTP/2, etc.) as static libraries. This can be challenging to set up correctly across different platforms and can significantly increase the executable size.

The provided Makefile primarily sets up for dynamic linking against libcurl and its dependencies (OpenSSL, zlib) as provided by vcpkg (or system libraries). If your vcpkg triplet is a static one (e.g., x64-linux-static), vcpkg itself will attempt to provide static libraries, and the linker might pick them up. You might need to add CXXFLAGS += -DCURL_STATICLIB when compiling your code if you are linking libcurl statically.

For a truly portable, standalone executable with static linking of all dependencies, you would typically:

Ensure vcpkg builds static versions of curl and all its dependencies for your target triplet (e.g., vcpkg install curl:x64-linux-static openssl:x64-linux-static zlib:x64-linux-static).

Modify the Makefile's LDFLAGS and LIBS to explicitly link all necessary .a files in the correct order. This often requires inspecting the dependencies of libcurl.a (e.g., using pkg-config --static --libs libcurl if available and configured for static libs, or manually identifying them).

This level of static linking is advanced and platform-dependent. The current Makefile provides a good starting point for typical development and deployment where dynamic libraries are acceptable.
